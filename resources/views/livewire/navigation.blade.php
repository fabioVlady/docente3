<nav class="bg-gray-800" x-data="{open:false}">
          ...
          <div class="hidden sm:block sm:ml-6">
            <div class="flex space-x-4">
              ...
              {{-- @foreach ($facultads as $facultad)
                <a href="#" class="text-gray-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">{{ $facultad->nombre_facultad }}</a>
              @endforeach --}}
          ...
          <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
            ...
            <div class="ml-3 relative" x-data="{open: false}">
              ...
              <div x-show="open" x-on:click.away="open=false" class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
                ...
                @can('admin.home')
                  <a href="{{route('admin.home')}}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem" tabindex="-1" id="user-menu-item-0">Dashboard</a>
    ...
    <div class="sm:hidden" id="mobile-menu" x-show="open" x-on:click.away="open = false">
      <div class="px-2 pt-2 pb-3 space-y-1">
        ...
        {{-- @foreach ($facultads as $facultad)
          <a href="#" class="text-gray-300 hover:bg-gray-700 hover:text-white block px-3 py-2 rounded-md text-base font-medium">{{$facultad->nombre_facultad}}</a>
        @endforeach --}}
      </div>
    </div>
  </nav>
